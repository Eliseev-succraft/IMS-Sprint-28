({
    ...
    writeOff: function (cmp, dataMap) {
        ...
        let action = cmp.get('c.loanWriteOff');
        action.setParams({
            dataMap: dataMap
        });
        action.setCallback(this, function (response) {
            ...
            if (state === 'SUCCESS') {
                that.begin('loanWriteOff-SUCCESS');
                let result = response.getReturnValue();
                that.log(result);
                cmp.find('message').showSuccessMessage('Writing Off completed.');
